...
const EmployeeJobCardUpload = () => {
  ...
  const handleFileUpload = (event) => {
    ...
    reader.onload = (e) => {
      ...
      if (headers.length < 6) {
        alert(
          "Invalid Excel format. Expected columns: Edatetime, UserID, Job_Target, Job_actual, Job_rejns, PPE",
        );
        return;
      }
  ...
  const handleJobCardUpload = async () => {
    ...
    try {
      const response = await axios.post(
        "https://192.168.2.54/api/jobcard-upload",
        data,
        {
  ...
  const downloadTemplate = () => {
    ...
    link.href = "https://192.168.2.54/download-templatejob";
